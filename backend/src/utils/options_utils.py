...
def get_index_symbol(broker_type: str, market: str) -> str:
    """
    Get the index symbol for a specific broker and market.
    """
    mappings = {
        "ZERODHA": {
            "NIFTY": "NIFTY 50",
        ...
        "ANGEL": {
            "NIFTY": "NIFTY", # Angel One uses NIFTY for the index
            "BANKNIFTY": "BANKNIFTY",
            "FINNIFTY": "FINNIFTY"
        },
        "MOCK": {
    ...
    broker_upper = str(broker_type).upper()
    if "ANGEL" in broker_upper: broker_upper = "ANGEL"
    elif "ZERODHA" in broker_upper: broker_upper = "ZERODHA"
    else: broker_upper = "ZERODHA" # Default
        
    market_upper = str(market).upper()
    return mappings[broker_upper].get(market_upper, market_upper)
...
def estimate_option_premium(spot_price: float, strike: int, option_type: str,
    ...
    return round(max(premium, 5.0), 2)  # Minimum Rs5 premium
